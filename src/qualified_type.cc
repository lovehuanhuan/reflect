...
bool QualifiedType::CanCast(const QualifiedType& t) const{
    ...
    if (type->Is(t.type)){
        if (pointerCount == 0 && t.pointerCount == 0 && !t.IsReference())
            return true;
        if (pointerCount == t.pointerCount && (!isConst || t.isConst) && 
            (!isVolatile || t.isVolatile) && (referType == ReferType::None || 
            (referType == ReferType::Refer && t.referType != ReferType::None) || 
            t.referType == ReferType::RightRefer))
            return true;
    }
